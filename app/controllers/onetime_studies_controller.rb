class OnetimeStudiesController < ApplicationController
  ...
  def index
    ...
    @count  = OnetimeStudy.distinct.notnil().includes(:pc_name, :world, :party).groups(action_name, params).ransack(params[:q]).result.hit_count()
    @search = OnetimeStudy.distinct.notnil().includes(:pc_name, :world, :party).groups(action_name, params).aggregations(action_name, params).having_order(params).page(params[:page]).ransack(params[:q])
