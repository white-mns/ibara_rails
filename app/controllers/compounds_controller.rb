class CompoundsController < ApplicationController
  ...
  def index
    placeholder_set
    param_set
    @count	= Compound.notnil().includes(:pc_name, :world, :compound_result).compound_include(params).groups(params).search(params[:q]).result.count().keys().size
    @search	= Compound.notnil().includes(:pc_name, :world, :compound_result).compound_include(params).groups(params).for_group_select(params).page(params[:page]).search(params[:q])
