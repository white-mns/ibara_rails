class CookPassivesController < ApplicationController
  ...
  def index
    ...
    @count	= CookPassive.notnil().includes(:pc_name, :world, :skill).search(params[:q]).result.hit_count()
    @search	= CookPassive.notnil().includes(:pc_name, :world, :skill).page(params[:page]).search(params[:q])
