class SkillsController < ApplicationController
  ...
  def index
    placeholder_set
    param_set
    @count	= Skill.distinct.notnil().includes(:pc_name, :world, :party, :status).groups(params).ransack(params[:q]).result.hit_count()
    @search	= Skill.distinct.notnil().includes(:pc_name, :world, :party, :status).groups(params).total(params).having_order(params).page(params[:page]).ransack(params[:q])
